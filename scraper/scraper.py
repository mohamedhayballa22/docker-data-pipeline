...
if not GOOGLE_API_KEY:
    logger.warning(
        "GOOGLE_API_KEY environment variable not set. Skill extraction will be skipped."
    )
else:
    try:
...
def scrape_linkedin_jobs(
    ...
    for i, job_title_query in enumerate(job_titles):
        ...
        try:
            ...
            logger.error(
                f"Could not URL-encode search terms: {e}. Skipping query '{job_title_query}'."
            )
            continue
        ...
        for page in range(num_pages):
            if max_jobs is not None and jobs_scraped_for_title >= max_jobs_for_title:
                logger.info(
                    f"Reached maximum jobs limit ({max_jobs_for_title}) for title '{job_title_query}'. Skipping remaining pages."
                )
                break
            ...
            try:
                ...
                if not job_cards:
                    logger.info(
                        f"No job cards found on page {page + 1} for '{job_title_query}'. Stopping search for this title."
                    )
                    break

                logger.info(
                    f"Found {len(job_cards)} potential jobs on page {page + 1}. Processing..."
                )

                for card in job_cards:
                    if (
                        max_jobs is not None
                        and jobs_scraped_for_title >= max_jobs_for_title
                    ):
                        logger.info(
                            f"Reached maximum jobs limit ({max_jobs_for_title}) for title '{job_title_query}'. Skipping remaining cards."
                        )
                    ...
                    if job_data["url"]:
                        ...
                        if job_description:
                            ...
                            logger.info(
                                f"Could not retrieve description for job: {job_data.get('title', 'N/A')}"
                            )
                    else:
                        logger.info(
                            f"Skipping description/skills fetch due to missing/invalid URL for job: {job_data.get('title', 'N/A')}"
                        )
                    ...
                    if job_data.get("title") and job_data.get("company"):
                        ...
                        logger.info(
                            f"Processed: {job_data['title']} at {job_data['company']}"
                        )
                    else:
                        logger.info(
                            "Skipping card - missing essential info (title or company)."
                        )

                logger.info(
                    f"Finished processing page {page + 1} for '{job_title_query}'."
                )

            except requests.exceptions.Timeout:
                logger.error(
                    f"Timeout requesting search page {page + 1} for '{job_title_query}'. Stopping for this query."
                )
                break
            except requests.exceptions.RequestException as e:
                logger.error(
                    f"HTTP error scraping search page {page + 1} for '{job_title_query}': {e}. Stopping for this query."
                )
                break
            except Exception as e:
                logger.error(
                    f"An unexpected error occurred on search page {page + 1} for '{job_title_query}': {e}"
                )
                break
            ...
            if page < num_pages - 1:
                ...
                logger.info(
                    f"Sleeping for {sleep_time:.1f} seconds before next page..."
                )
                time.sleep(sleep_time)

        logger.info(
            f"Scraped a total of {jobs_scraped_for_title} jobs for title '{job_title_query}'."
        )
